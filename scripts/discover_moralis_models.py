...
Every registered collection contract is inspected. `collections.contract` is
only the primary convenience address and must not hide migrated/secondary
contracts recorded in the canonical `contracts` table.
"""
...
async def run(args):
    ...
    for row in rows:
        for contract in row.get("contracts") or []:
            chain=text(contract.get("chain")).lower(); address=text(contract.get("address")).lower()
            if chain in CHAIN_MAP and valid_evm_address(address):
                targets.append({**row,"chain":chain,"contract":address,"is_primary":bool(contract.get("is_primary"))})
    ...
    async with MoralisClient(max_concurrency=args.concurrency) as client:
        async def one(row):
            async with sem:
                ...
                except Exception as exc: return {"catalogId":row["id"],"name":row["name"],"chain":row["chain"],"contract":row["contract"],"isPrimary":row.get("is_primary",False),"error":str(exc)[:500],"nfts":[]}
                out=[]
                ...
                return {"catalogId":row["id"],"name":row["name"],"chain":row["chain"],"contract":row["contract"],"isPrimary":row.get("is_primary",False),"error":None,"nfts":out}
        contract_results=await asyncio.gather(*(one(r) for r in targets))
    grouped={}
    for result in contract_results:
        group=grouped.setdefault(result["catalogId"],{"catalogId":result["catalogId"],"name":result["name"],"contracts":[],"error":None,"nfts":[]})
        group["contracts"].append({"chain":result["chain"],"contract":result["contract"],"isPrimary":result["isPrimary"],"error":result["error"],"nfts":result["nfts"]})
        group["nfts"].extend({**nft,"chain":result["chain"],"contract":result["contract"]} for nft in result["nfts"])
    collections=[]
    for group in grouped.values():
        errors=[f"{item['contract']}: {item['error']}" for item in group["contracts"] if item["error"]]
        group["error"]="; ".join(errors) if errors else None
        collections.append(group)
    collections.sort(key=lambda row:(text(row.get("name")).lower(),text(row.get("catalogId"))))
    candidates=[(c,n) for c in collections for n in c["nfts"]]
    return {"schema":"moralis-model-discovery-v2","generatedAt":now_iso(),"policy":"lead-only; .vrm/.glb and unsupported_media are not VRM proof; every registered contract is sampled","summary":{"collectionsInspected":len(collections),"contractsInspected":len(contract_results),"collectionsWithErrors":sum(bool(c["error"]) for c in collections),"nftsWithSignals":len(candidates),"modelCandidates":sum(len(n["modelCandidates"]) for _,n in candidates),"unsupportedMediaNfts":sum(bool(n["unsupportedMedia"]) for _,n in candidates)},"collections":collections}
