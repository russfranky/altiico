...
export default async function AvatarSetsPage() {
  const sets = await localCatalogAdapter.listSets();
  return (
    <ProductShell section="EXPLORE / LOCAL FIXTURE">
      <div className="bentoGrid catalogGrid">
        ...
        <BentoPanel className="filterCell" label="FILTER / FOUNDATION">
          <div className="filterReadout"><span>QUERY</span><b>ALL SETS</b></div><div className="filterReadout"><span>STATE</span><b>ANY</b></div><div className="filterReadout"><span>SOURCE</span><b>LOCAL FIXTURE</b></div>
        </BentoPanel>
        {sets.map((set, index) => (
        ...
        <BentoPanel className="evidenceCell" label="EVIDENCE CONTRACT"><p>Fixture data proves route and state behavior only. Live catalog evidence will enter through the adapter after review.</p></BentoPanel>
      </div>
    </ProductShell>
  );
}
